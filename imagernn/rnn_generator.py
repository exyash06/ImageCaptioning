  ...
  def forward(Xi, Xs, model, params, **kwargs):
    ...
    if drop_prob_encoder > 0: 
      if not predict_mode:
        ...
        Us = (np.random.rand(*(Xs.shape)) < (1 - drop_prob_encoder)) * scale 
        Xs *= Us 
    ...
    for t in xrange(n):
      ...
      if not rnn_feed_once or t == 0:
        ...
        H[t] = np.maximum(Xi + Xsh[t] + prev.dot(Whh) + bhh, 0) 
      else:
        H[t] = np.maximum(Xsh[t] + prev.dot(Whh) + bhh, 0) 
...
def ymax(y):
  """Helper function here that takes unnormalized logprobs """
  y1 = y.ravel()
  maxy1 = np.amax(y1)
  e1 = np.exp(y1 - maxy1) 
  p1 = e1 / np.sum(e1)
